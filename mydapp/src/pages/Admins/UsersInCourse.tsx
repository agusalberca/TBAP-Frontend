...
export const UsersInCourse: React.FC = withAdminProtection(() => {
  ...
  const query = useQuery('getUsersOfCourse', () =>
  ...
  const {
    isOpen: isOpenModalSingular,
    onOpen: onOpenModalSingular,
    onClose: onCloseModalSingular,
  } = useDisclosure();
  const {
    isOpen: isOpenModalMassive,
    onOpen: onOpenModalMassive,
    onClose: onCloseModalMassive,
  } = useDisclosure();
      ...
      <Modal isOpen={isOpenModalMassive} onClose={onCloseModalMassive}>
        {currentStep === 1 && (
          <form onSubmit={formikMassive.handleSubmit}>
            <ModalOverlay />
            <ModalContent>
              <ModalHeader>{t('Add a file with users')}</ModalHeader>
              <ModalCloseButton />
              <ModalBody>
                <Text>
              ...
              <ModalFooter>
                ...
                <Button onClick={onCloseModalMassive}>{t('Cancel')}</Button>
              </ModalFooter>
        ...
        {currentStep === 2 && (
          <>
            <ModalOverlay />
            <ModalHeader>{t('Results')}</ModalHeader>
            <ModalContent>
                  ...
                  <>
                    {responseMassiveEmails.Accepted.amount > 0 && (
                      <div>
                        ...
                        {responseMassiveEmails.Accepted.emails.map(
                          (info, index) => (
                    ...
                    {responseMassiveEmails.Denied && (
                      <>
                        {responseMassiveEmails.Denied.emails
                          .already_sended_errors.length > 0 && (
                          <>
                            <div
                              style={{
                                display: 'flex',
                            ...
                            >
                              ...
                              <Text marginLeft={1} fontWeight={500}>
                                {t('Errors')} (
                                {responseMassiveEmails.Denied.amount})
                              </Text>
                            </div>
                            <br />
                            <div>
                              <h5>{t('Already Sent Errors')}:</h5>
                              {responseMassiveEmails.Denied.emails.already_sended_errors.map(
                                (info, index) => (
                                  <Text key={index} marginLeft={2}>
                                    {info}
                                  </Text>
                        ...
                        <br />
                        {responseMassiveEmails.Denied.emails
                          .is_already_in_errors.length > 0 && (
                          <>
                            <div>
                              <h5>{t('Is Already In Errors')}:</h5>
                              {responseMassiveEmails.Denied.emails.is_already_in_errors.map(
                                (info, index) => (
                                  <Text key={index} marginLeft={2}>
                                    {info}
                                  </Text>
              ...
              <ModalFooter>
                <Button onClick={handleCloseModalMassive}>{t('Close')}</Button>
              </ModalFooter>
            </ModalContent>
          </>
        )}
